...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder completed:(SDWebImageCompletedBlock)completedBlock{
    ...
    _imageRequest = [[ASIHTTPRequest alloc] initWithURL:url];
    [_imageRequest setDownloadCache:[ASIDownloadCache sharedCache]];
    [_imageRequest setCachePolicy:ASIOnlyLoadIfNotCachedCachePolicy];
    [_imageRequest setCacheStoragePolicy:ASICacheForSessionDurationCacheStoragePolicy];
    [_imageRequest setSecondsToCache:3600*24];
    [_imageRequest setStartedBlock:^(void){
    }];
    
    __block __weak ASIHTTPRequest *imageRequest = _imageRequest;
    __block __weak SCGIFImageView *wself = self;
    [_imageRequest setCompletionBlock:^(void){
        if (!wself) return;
        NSData  *data = imageRequest.responseData;
        if ([SCGIFImageView isGifImage:data]) {
            [wself decodeGIF:data];
            [wself loadImageData];
            completedBlock([wself getFrameAsImageAtIndex:0],imageRequest.error,0);
        }else{
            UIImage  *image = [UIImage imageWithData:data];
            [wself setImage:[UIImage imageWithData:data]];
            completedBlock(image,imageRequest.error,0);
        }
    }];
    [_imageRequest setFailedBlock:^(void){
        completedBlock(nil,imageRequest.error,0);
    }];
