...
- (void) addFilters:(NSArray *)filters{
    [self.filters addObjectsFromArray:filters];
    [self _refreshFilters];
}
...
- (UIImage *) currentFilteredFrame {
    GPUImageOutput *imageOut = [_filters count]>0?[_filters lastObject]:self.input;
    return [imageOut imageFromCurrentlyProcessedOutput];
}

- (UIImage *) currentFilteredFrameWithOrientation:(UIImageOrientation)orientation{
    GPUImageOutput *imageOut = [_filters count]>0?[_filters lastObject]:self.input;
    return [imageOut imageFromCurrentlyProcessedOutputWithOrientation:orientation];
}

- (CGImageRef) newCGImageFromCurrentFilteredFrame {
    GPUImageOutput *imageOut = [_filters count]>0?[_filters lastObject]:self.input;
    return [imageOut newCGImageFromCurrentlyProcessedOutput];
}
