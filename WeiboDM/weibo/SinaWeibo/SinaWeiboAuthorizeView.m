...
- (id)init {
    if ((self = [super init])) {
        ...
        whiteBGImageView = [[UIImageView alloc] init];
        whiteBGImageView.backgroundColor = [UIColor whiteColor];
        whiteBGImageView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        [self addSubview:whiteBGImageView];
        [whiteBGImageView release];
...
//        [self addSubview:closeButton];
        ...
        modalBackgroundView.userInteractionEnabled = YES;
        modalBackgroundView.layer.masksToBounds = YES;
        modalBackgroundView.layer.cornerRadius = 5;
        modalBackgroundView.alpha = 0;
        [modalBackgroundView setAutoresizingMask:UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth];
        [modalBackgroundView setBackgroundColor:[[UIColor darkGrayColor] colorWithAlphaComponent:0.8]];
        
        UITapGestureRecognizer *sinaLoginBGGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(sinaLoginViewClose:)];
        [modalBackgroundView addGestureRecognizer:sinaLoginBGGesture];
...
- (void)sinaLoginViewClose:(id)sender
{
    [self cancel];
}
...
- (void)updateWebOrientation {
    ...
    if (UIInterfaceOrientationIsLandscape(orientation)) {
        ...
        whiteBGImageView.frame = webView.frame;
    }
    else {
        [webView stringByEvaluatingJavaScriptFromString:
...
- (void)webViewDidFinishLoad:(UIWebView *)aWebView {
    [aWebView stringByEvaluatingJavaScriptFromString:
     [NSString stringWithFormat:@"document.querySelector('meta[name=viewport]').setAttribute('content', 'width=%d;', false); ", (int) aWebView.frame.size.width]];
    whiteBGImageView.frame = aWebView.frame;
    [self hideIndicator];
}
